...
# GoJay
**Package is currently at version 0.10.8 and still in development, it does not cover escaped unicode syntax eg: "\u2605", otherwise it is safe to use in production**
...
* DecodeObject
```go
func (dec *Decoder) DecodeObject(v UnmarshalerJSONObject) error
```
* DecodeArray
```go
...
func (u *users) MarshalJSONArray(dec *Decoder) {
	for _, e := range u {
        enc.AddObject(e)
    }
}
